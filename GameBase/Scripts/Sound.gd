...
func _SetBusVolumes() -> void:
	...
	AudioServer.set_bus_volume_db(sfx_index, linear_to_db(GameMaster.gameData.maleVolume))
	AudioServer.set_bus_volume_db(sfx_index, linear_to_db(GameMaster.gameData.femaleVolume))
